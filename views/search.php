...
<?php else: ?>
	...
	<?php foreach(getUsersLike($search, $db) as $row): ?>
				<div class="results_entry">
					<h4>
						<a href="profile.php?id=<?php echo htmlentities($row['user_id'], ENT_QUOTES, 'utf-8'); ?>"><?php echo htmlentities(getUsername($row['user_id'], $db), ENT_QUOTES, 'utf-8'); ?>
						</a>
					</h4>
				</div>
	<?php endforeach; ?>
...
<?php else: ?>
	...
	<?php foreach(getProjectsLike($search, $db) as $row): ?>
				<div class="results_entry">
					<h4>
						<a href="project.php?id=<?php echo htmlentities($row['project_id'], ENT_QUOTES, 'utf-8'); ?>"><?php echo htmlentities(getProjectName($row['project_id'], $db), ENT_QUOTES, 'utf-8'); ?>
						</a>
					</h4>
				</div>
	<?php endforeach; ?>
